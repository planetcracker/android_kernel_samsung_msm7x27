...
static void cpufreq_dynamic_interactive_timer_resched(
 ...
 * function.
 */
static void cpufreq_dynamic_interactive_timer_start(int cpu)
{
	struct cpufreq_dynamic_interactive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_dynamic_interactive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
static void cpufreq_dynamic_interactive_timer(unsigned long data)
{
	...
	if (pcpu->target_freq >= hispeed_freq &&
		...
		trace_cpufreq_dynamic_interactive_notyet(
	...
	if (pcpu->target_freq == new_freq) {
		trace_cpufreq_dynamic_interactive_already(
			data, cpu_load, pcpu->target_freq,
			pcpu->policy->cur, new_freq);
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_dynamic_interactive_timer_resched(pcpu);
...
static void cpufreq_dynamic_interactive_idle_end(void)
{
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		...
		cpufreq_dynamic_interactive_timer(smp_processor_id());
	}
...
static int cpufreq_dynamic_interactive_notifier(
...
static struct notifier_block cpufreq_notifier_block = {
	.notifier_call = cpufreq_dynamic_interactive_notifier,
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			cpufreq_dynamic_interactive_timer_start(j);
...
static void cpufreq_dynamic_interactive_nop_timer(unsigned long data)
{
}
...
static int __init cpufreq_dynamic_interactive_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		pcpu->cpu_slack_timer.function = cpufreq_dynamic_interactive_nop_timer;
