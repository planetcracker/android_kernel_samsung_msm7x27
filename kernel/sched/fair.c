			...
			int *balance, struct sg_lb_stats *sgs)
{
	...
	if (sgs->sum_nr_running) {
		avg_load_per_task = sgs->sum_weighted_load * SCHED_POWER_SCALE / group->sgp->power;
				...
				int this_cpu, unsigned long *imbalance)
{
	...
	scaled_this_load_per_task = sds->this_load_per_task * SCHED_POWER_SCALE;
	...
	pwr_now += sds->this->sgp->power *
			min(scaled->this_load_per_task, sds->this_load);
	...
	pwr_move += sds->this->sgp->power *
			min(scaled->this_load_per_task, sds->this_load + tmp);
